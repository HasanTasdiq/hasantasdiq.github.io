---
title: "Research"
draft: false
type: 'bookcase'
bookcase_list_sections: true
bookcase_list_pages: true
---

## Research Interests
I’m currently exploring Large Language Models (LLMs), AI, and Edge Computing -- figuring out how to make AI smarter, faster, and more efficient on edge devices. My work focuses on bringing powerful AI models closer to where they’re needed, so they can run efficiently outside the cloud. A few of my specific interests include:

- Large Language Models (LLMs)
- Edge Deployment and Inference Optimization
- Edge AI
- AI Acceleration

---

## Ongoing Projects
- **LLM-Powered Feature Extraction for Edge**: Using LLMs to extract relevant features from text data for edge devices.  
  - [Project page](/projects/effe)  

---

## Past Projects
- **Context Sensitive Fuzzer Evaluation**: A study to evaluate the ability of modern fuzzing tools on generating context-sensitive inputs.  
  - [Project page](/projects/fuzzeval)

- **SeQR: Secure and Simple Enterprise Wi-Fi Configuration**: A system to securely configure enterprise Wi-Fi on mobile devices using QR codes.
  - [Project page](/projects/seqr)


- **Socioeconomic Factors and Global Disease Outbreaks**: An analysis of the relationships between socioeconomic factors' influence over  global disease outbreaks.
  - [Project page](/projects/diseaseoutbreaks)